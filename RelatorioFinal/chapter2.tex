...
\subsection{Manutenção da aplicação e \textit{Backoffice}}\label{subsec:manutencao_e_backoffice}

Com o objetivo de realizar operações de manutenção e operações que requerem privilégio superior ao de um utilizador comum foi implementada a aplicação \textit{BackOffice}. 
Um utilizador com permissões de administrador ou de \textit{owner} tem acesso ao \textit{BackOffice} onde pode criar ou remover categorias, analisar as necessidades criadas pela comunidade empresarial e escolher se as mesmas são aceites ou eliminadas, 
...
apresentar informação pertinente de uma forma apelativa. Este ecrã começa por apresentar o número total de utilizadores registados na aplicação e o total de necessidades criadas até ao momento organizadas por nível de prioridade(Low, Medium e High). Ao carregar num destes niveis prioridade especificados no ecrã o utilizadoré redirecionado para o ecrã das necessidades, apresentando a lista das mesmas com o filtro correspondente à prioridade selecionada. Contém um gráfico circular que faz uma estatística relativa ao número de necessidades que existem em cada categoria em que a seleção de uma das categorias deste gráfico redirecionará o utilizador para o ecrã das necessidades, apresentando a lista das mesmas com o filtro correspondente à categoria selecionada. Apresenta o top 5 de necessidades criadas mais recentemente e aceites pelo administrador, em que cada uma tem um link que redireciona o utilizador para o detalhe da mesma e por fim mostra ainda três top 5 de utilizadores que têm mais necessidades criadas por nível de prioridade. 
...
%UsersManagers
%Foi desenvolvido um ecrã chamado ManageUsers que permite aos utilizadores com as permissões de admin e owner fazer uma gestão de todos os utilizadores da aplicação.
%Para isso, foram criadas duas tabelas. Uma delas mostra todos os utilizadores com apenas a role de registado e a possibilidade de para cada um, ser promovido a 
%administrador, e a outra mostra todos os utilizadores com a role de administrador ou owner e a possibilidade de para cada um, ser despromovido de administrador com exceção do onwer. 
%Neste caso, apenas os utilizadores com a role de owner podem despromover administradores a utilizadores registados.


No contexto do uma necessidade, existem os seguintes roles:

Autor - Utilizador que criou a necessidade. Tem as permissões de poder editar e eliminar a necessidade.

Participante - Utilizador que se candidata durante a fase de participantes e se aceite pode, opcionalmente, participar também nos recursos associados à mesma.

Colaborador - Utilizador que se candidatou a uma necessidade durante a fase de colaborador. Se aceite por parte do autor, 
adquire as mesmas permissões de um participante comum e adquire também as permissões de poder editar a necessidade com exceção da data do evento e de aceitar/rejeitar participantes desta necessidade. 
