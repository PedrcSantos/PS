...
Neste ecrã irá ser pedido ao utilizador permissões de utilização da câmara para realizar uma leitura contínua de QR codes pressionando no botão \textit{Scan QR Codes} e de seguida no \textit{Capture} para capturar o conteúdo visto na câmara, se existir um QR code irá ser apresentado uma mensagem de sucesso caso contrário uma mensagem de erro a avisar o utilizador que não foi possível encontrar nenhum QR code.
